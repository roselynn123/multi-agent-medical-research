const GPTResearcher = (() => {
  ...
  const listenToSockEvents = () => {
    ...
    socket.onmessage = (event) => {
      ...
      // 重点：把当前真正通畅的 websocket 实例直接存到 window 全局变量中！
      window.activeWebSocket = this.socket || event.target;

      const data = JSON.parse(event.data)
      console.log("Received message:", data);  // Debug log

      // ------------------- 🛑 插入 HITL 消息拦截 -------------------
      if (data.type === "human_feedback_request" && data.content === "subqueries_review") {
          const subQueries = data.output.sub_queries || [];
          if (typeof openHitlModal === "function") {
              openHitlModal(subQueries);
          }
          return; // 直接拦截，阻止打印普通日志
      }
      // -----------------------------------------------------------
