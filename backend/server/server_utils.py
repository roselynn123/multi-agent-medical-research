...
# 全局 HITL 锁与数据暂存器
hitl_feedback_event = asyncio.Event()
hitl_user_data = {}
    ...
    try:
        while True:
            try:
                ...
                # ------------------- 🛑 重点 1：优先拦截 human_feedback -------------------
                elif data.strip().startswith("human_feedback"):
                    logger.info("Processing human_feedback command")
                    try:
                        # 解析前端发过来的 JSON 数据
                        # 格式示例: "human_feedback {"sub_queries": ["Q1", "Q2"]}"
                        payload_str = data.strip()[len("human_feedback"):].strip()
                        if payload_str:
                            parsed_data = json.loads(payload_str)
                        else:
                            parsed_data = {}
                            
                        # global hitl_user_data, hitl_feedback_event
                        # hitl_user_data = parsed_data
                        # hitl_feedback_event.set() # 🔑 解锁正在等待的 Agent 主线程！

                        # ✅ 新代码（原址修改字典，保证全局共享一致）
                        hitl_user_data.clear()        # 1. 清空旧数据
                        hitl_user_data.update(parsed_data) # 2. 把新数据填入同一个内存地址

                        logger.info(f"🔍 [Server] 已成功将数据更新至全局 hitl_user_data: {hitl_user_data}")

                        hitl_feedback_event.set()     # 3. 解锁 Agent
                        
                        await websocket.send_json({
                            "type": "logs",
                            "content": "info",
                            "output": "✅ 已收到人工干预反馈，研究继续进行..."
                        })
                    except Exception as e:
                        logger.error(f"解析 human_feedback 失败: {e}")
